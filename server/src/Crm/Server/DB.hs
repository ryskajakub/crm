...
instance ColumnToRecord 
    ...
    MachineMapped where
  convert tuple = let
    machineTuple = $(modT 10 4) (fmap dayToYmd) tuple
    in (M.MachineId $ $(proj 10 0) tuple, C.CompanyId $ $(proj 10 1) tuple, CP.ContactPersonId `fmap` $(proj 10 2) tuple, 
      MT.MachineTypeId $ $(proj 10 3) tuple, 
      (uncurryN $ const $ const $ const $ const M.Machine) machineTuple)
